...
分区可以应对很多前沿应用，分区是建立T级数据库或高可用系统的关键技术。
##### 分区提高性能
...
#### 分区与管理
分区允许表和索引分解到更小的，更易管理的单位，让数据库管理员可以分而治之。有了分区，维护操作可以只专注于表的个别分区。例如，管理员可以只备份一个分区。对于整库对象的维护操作，可以分解到单个分区为基础。将维护过程分解到更易管理的区间。

分区在管理上的典型应用是支持数据仓库的“滚动窗口”加载进程。假如DBA按周将新数据加载到表中，可以将该表按周分区。加载操作只需简单的通过分区交换加载一个新分区，添加一个分区比修改整个表更高效，因为DBA不需要修改其他分区。
